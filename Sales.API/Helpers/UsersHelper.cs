namespace Sales.API.Helpers
{
    ...
    public class UsersHelper : IDisposable
    {
        ...
        public static Responce CreateUserASP(UserRequest userRequest)
        {
            try
            {
                var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(userContext));
                ...
                if (oldUserASP != null)
                {
                    return new Responce { IsSuccess = false, Message = "001. User already exists.", };
                }
                var userASP = new ApplicationUser { Email = userRequest.EMail, UserName = userRequest.EMail, PhoneNumber = userRequest.Phone, };
                var result = userManager.Create(userASP, userRequest.Password);
                if (result.Succeeded)
                {
                    var newUserASP = userManager.FindByEmail(userRequest.EMail);
                    userManager.AddClaim(newUserASP.Id,
                    new System.Security.Claims.Claim(ClaimTypes.GivenName, userRequest.FirstName));
                    ...
                    return new Responce { IsSuccess = true, };
                }
                var errors = string.Empty; foreach (var error in result.Errors)
                {
                    errors += $"{error}, ";
                }
                return new Responce { IsSuccess = false, Message = errors, };
            }
            catch (Exception ex)
            {
        ...
        public static void CreateUserASP(string email, string roleName)
        {
            ...
            if (userASP == null)
            {
                userASP = new ApplicationUser { Email = email, UserName = email, };
        ...
        public static async Task PasswordRecovery(string email)
        {
            ...
            if (response.Succeeded)
            {
                ...
                var body = string.Format(@"  <h1>Sales App - Recuperación de contraseña</h1>  <p>Su nueva contraseña es: <strong>{0}</strong></p>  <p>Por favor no olvide cambiarla por una de fácil recordación", newPassword);
        ...
        public void Dispose()
        {
            userContext.Dispose(); db.Dispose();
        }
